...
class NJE:
        def __init__(self, rhost='', ohost='', host='', port=0, password='', rip='8.8.8.8'):
                ...
                self.own_node   = b'\x01'#chr(0x01) # Node is default 1. Can be changed to anything
                self.sequence   = b'\x80'#0x80
                if host:
        ...
        def connect(self, host, port=0, timeout=1):
                ...
                #except ssl.SSLError, e:
                        ...
                        try:
                                ...
                                self.msg('Non-SSL Connection Failed: %r')
                                return False
                #except Exception, e:
        ...
        def disconnect(self):
                ...
                sock = self.sock
                self.sequence = b'\x80' #reset sequence
                self.connected = False
                self.sock = 0
                if sock:
                        sock.close()
        ...
        def INC_SEQUENCE(self):
                prev = self.sequence
                #self.msg('INC_SEQUENCE old self.sequence: ' + str(self.sequence))
                self.sequence = struct.pack('B',(int(self.sequence.hex(), base=16) & int(b'\x0F'.hex(), base=16)) + (1 | int(b'\x80'.hex(), base=16))) #self.sequence = (self.sequence & 0x0F)+1|0x80
                #self.msg('INC_SEQUENCE new self.sequence: ' + str(self.sequence))
                self.msg("Incremented sequence number from " + prev.hex() + ' to ' + self.sequence.hex())
        ...
        def EbcdicToAscii(self, s):
                ...
                if isinstance(s, str):
                    ...
                    self.msg(s)
                        ...
                        if isinstance(s, int):
                                return bytes([s]).decode('cp500').encode('utf-8')
                        else:
                                return s.decode('cp500').encode('utf-8')
        ...
        def initiate(self):
                ...
                nje_packet = ( self.TYPE + self.RHOST + self.RIP + self.OHOST + self.OIP + self.R )

                self.msg("Sending  >> TYPE: " + self.EbcdicToAscii(self.TYPE).decode('utf-8') +
                             " RHOST: " + self.EbcdicToAscii(self.RHOST).decode('utf-8') +
                             " OHOST: " + self.EbcdicToAscii(self.OHOST).decode('utf-8'))
                ...
                if bytes(buff[32]) == b'':
                        ...
                        #,BELV comm 10.02.2020 bR     = struct.unpack("b", bytes(buff[32]))[0]
                        bR = bytes(buff[32])
                ...
                if bR == 4:
                        print('[!] Incorrect RHOST (' + self.EbcdicToAscii(self.RHOST).strip() + ") for OHOST: " + self.EbcdicToAscii(self.OHOST).strip() + "[!] Or RHOST already connected to OHOST")
                        self.disconnect()
                        return False
                elif bR == 1:
        ...
        def signon(self):
                ...
                if not self.connected:
                        return False
                self.msg('sequence: ' + str(self.sequence))
                self.msg("Sequence is: " + str(self.sequence))
                self.msg("Own Node   : " + str(self.own_node))
                self.msg("Dest Node  : " + str(self.own_node))
                self.signed_on = True
                return True
        ...
        def sendNMR(self, message, cmd=False, user=''):
                ...
                if cmd:
                        ...
                        NMRTO    = self.OHOST + self.own_node # This is TO node name and number
                ...
                NMRML   = chr(len(NMRMSG)).encode('utf-8')
                if isinstance(NMROUT, str):
                        NMROUT = NMROUT.encode('utf-8')
        ...
        def sendNJE(self, RCB, SRCB, data, compress=False):
                ...
                #self.msg('BCB: ' + str(BCB))
                #self.msg('FCS: ' + str(FCS))
                #self.msg('TTR: ' + str(TTR))
                #self.msg('nje_record: ' + str(nje_record))
                #self.msg('DS + BCB + FCS + nje_record: ' + str(DS + BCB + FCS + nje_record))
                #self.msg('TTR + DS + BCB + FCS + nje_record: ' + str(TTR + DS + BCB + FCS + nje_record))
        ...
        def padding(self, word):
                ...
                return self.AsciiToEbcdic(word.upper()) + self.AsciiToEbcdic(SPACE * (8-len(word)))
        ...
        def makeTTB(self, data):
                ...
                #self.msg('makeTTB bytes(struct.pack(">H", len(data)+8+4)): ' + str(bytes(struct.pack('>H', len(data)+8+4))))
                #self.msg('makeTTB data: ' + str(data))
                #self.msg('makeTTB return: ' + str(b"\x00\x00" + bytes(struct.pack('>H', len(data)+8+4)) +
                #            b"\x00\x00\x00\x00" + data + b"\x00\x00\x00\x00"))
                return (b"\x00\x00" + bytes(struct.pack('>H', len(data)+8+4)) +
                            b"\x00\x00\x00\x00" + data + b"\x00\x00\x00\x00")
        ...
        def makeTTR(self, data, eor=False):
                ...
                if eor:
                        return b"\x00\x00" + bytes(struct.pack('>H', len(data))) + data + b"\x00"
                else:
                        return b"\x00\x00" + bytes(struct.pack('>H', len(data))) + data
        ...
        def processData(self, data):
                ...
                self.msg('processData received_data: ' + str(received_data))
                return received_data

        def phex(self, stuff):
                hexed = binascii.hexlify(bytearray(stuff))
                return b''.join(hexed[i:i+2] for i in range(0, len(hexed), 2))
                #hexed = bytearray(stuff)
                #return "".join("%02x" % b for b in hexed)
        ...
        def process_RCB(self):
                ...
                for record in self.records:
                        ...
                        self.msg("SRCB1!!!!!!!!!!!!!!!!!!!!: %r", record['SRCB'])
                        self.msg("Data!!!!!!!!!!!!!!!!!!!!!: %r", record['Data'])
                        ...
                        if RCB == 0x00:
                                ...
                                NMR.append(data)
#                        elif (int.from_bytes(RCB, 'big') & 0x0F) == 0x08:
                        elif (RCB & 0x0F) == 0x08:
                                self.msg("Type: SYSIN record (98-F8)")
                                data = self.process_SYSIN(record)
                                SYSIN.append(data)
#                        elif (int.from_bytes(RCB, 'big') & 0x0F) == 0x09:
                        elif (RCB & 0x0F) == 0x09:
        ...
        def process_NCCR(self, record):
                ...
                SRCB = self.EbcdicToAscii(record['SRCB']).decode('utf-8')
                if SRCB == "I":
                        ...
                        self.msg("NCCIQUAL: %r", str(record['NCCIQUAL']))
        ...
        def dataset_headers(self, d):
                ...
                'NDHGFLG1' : d[52],
                'NDHGF1SP' : self.get_bit(d[52],7),
                'NDHGF1HD' : self.get_bit(d[52],6),
                'NDHGF1LG' : self.get_bit(d[52],5),
                'NDHGF1OV' : self.get_bit(d[52],4),
                'NDHGF1IN' : self.get_bit(d[52],3),
                'NDHGF1LC' : self.get_bit(d[52],2),
                'NDHGF1ST' : self.get_bit(d[52],1),
                'NDHGF1DF' : self.get_bit(d[52],0),
                'NDHGRCFM' : d[53],
                ...
                'NDHGFLG2' : d[100],
                'NDHGF2PR' : self.get_bit(d[100],7),
                'NDHGF2PU' : self.get_bit(d[100],6),
                'NDHGF2NM' : self.get_bit(d[100],5),
                'NDHGF2HB' : self.get_bit(d[100],4),
                'NDHGF2HA' : self.get_bit(d[100],3),
                'NDHGUCSO' : d[101],
                'NDHGUCSD' : self.get_bit(d[101],7),
                'NDHGUCSF' : self.get_bit(d[101],6),
                ...
                while len(d) > 1:
                        ...
                        if header == 0x8C:
                                ...
                                job.update( {
                                        ...
                                        'NDHTVERS' : d[1],
                                        'NDHTFLG1' : d[2],
                                        'NDHT1EN'  : self.get_bit(d[2],7),
                                        'NDHT1EXT' : self.get_bit(d[2],6),
                                        'NDHTSTYP' : d[3],
                                        'NDHTFLG2' : d[4],
                                        'NDHT2DFT' : self.get_bit(d[4],7),
                                        'NDHT2MLO' : self.get_bit(d[4],5),
                                        'NDHT2SHI' : self.get_bit(d[4],4),
                                        'NDHT2TRS' : self.get_bit(d[4],3),
                                        'NDHT2SUS' : self.get_bit(d[4],2),
                                        'NDHT2RMT' : self.get_bit(d[4],1),
                                        'NDHTPOEX' : d[5],
                                        'RESERVED' : d[6:8],
        ...
        def job_headers(self, d):
                ...
                'NJHGFLG1' : d[8],
                'NJHGF1PR' : self.get_bit(d[8],7),
                'NJHGF1CF' : self.get_bit(d[8],3),
                'NJHGF1CA' : self.get_bit(d[8],2),
                'NJHGF1PE' : self.get_bit(d[8],1),
                'NJHGF1NE' : self.get_bit(d[8],0),
                'NJHGPRIO' : d[9],
                'NJHGORGQ' : d[10],
                'NJHGJCPY' : d[11],
                'NJHGLNCT' : d[12],
                ...
                while len(d) > 1:
                        ...
                        if header == 0x8A :
                                ...
                                #d[7] is reserved
                                d = d[8:]
                                job['NJHTLENT'] = struct.unpack("b",bytes([d[0]]))[0]
                                job['NJHTVERS'] = struct.unpack("b",bytes([d[1]]))[0]
                                job['NJHTFLG1'] = d[2]
                                job['NJHTSTYP'] = d[3]
                                job['NJHTFLG2'] = d[4]
                                job['NJHTPOEX'] = d[5]
        ...
        def makeSYSIN_header(self, lines, jobnum, programmer, job_class, msg_class, job_name, acc, userid="ibmuser", group="sys1", passw=''):
                ...
                nje_header = ( b"\x00\xD4"+ b"\x00" + # Length + NJHGTYPE
                      b"\x00" + #NJHGMOD
                          ...
                          self.AsciiToEbcdic(msg_class) + #NJHGMCLS
                           b"\x40" + #NJHGFLG1
                           chr(9).encode('utf-8') + #NJHGPRIO
                           bytes([self.target_node]) + #NJHGORGQ
                           b"\x01" + #NJHGJCPY
                           b"\x00" + #NJHGLNCT
                           b"\x00" + #Reserved?
                           b"\x00\x00" + #NJHGHOPS
                           ...
                           (self.AsciiToEbcdic(programmer) + b'\x40' * (20-len(programmer))) + #NJHGPRGN
                           #("\x40" * 20) +
                           (b"\x40" * 8) + #NJHGROOM
                           (b"\x40" * 8) + #NJHGDEPT
                           (b"\x40" * 8) + #NJHGBLDG
                ...
                acc_header = (b"\x8D" + b"\x00" + b"\x00\x00" + b"\x00" + b"\x08" +
                                                        #NJHAJLEN                                             NJHAJAC1
                                                struct.pack(">h",len(acc) + 2) + b"\x01" + chr(len(acc)).encode('utf-8') + self.AsciiToEbcdic(acc) )
                ...
                self.msg('job_prefix' + str(self.phex(job_prefix)))
                self.msg('nje_header: ' + str(self.phex(nje_header)))
                self.msg('jes2_header' + str(self.phex(jes2_header)))
                self.msg('sched_header' + str(self.phex(sched_header)))
                self.msg('acc_header' + str(self.phex(acc_header)))
                self.msg('sec_header' + str(self.phex(sec_header)))
                self.msg('part2' + str(self.phex(part2)))

                self.msg('part1 + part2' + str(self.phex(part1) + self.phex(part2)))
        ...
        # This version implements compression better than IBM for some reason.
                ...
                self.msg("Raw Message before compression: %s", str(self.phex(buf)))
                ...
                while len(buf) > 0 and processed_bytes < 253:
                        if buf[0] == 0x40 and buf[1] == 0x40:
                                if c > 0: d += bytes([int(b'\xC0'.hex(), 16) + c]) + t # If we go straight from repeat char to repeat spaces this creates an extra char
                                t = b''
                                c = 1
                                while c < len(buf) and (buf[c] == 0x40 and (processed_bytes + c < 253)):
                                        if c == 31: break
                                        c += 1
                                d += bytes([int(b'\x80'.hex(), 16) + c])
                                #self.msg("Repeated %r %i times", buf[0], c)
                                buf = buf[c-1:]
                                processed_bytes += c
                                c = 0
                        elif len(buf) > 2 and buf[0] == buf[2] and buf[0] == buf[1]:
                                if c > 0: d += (bytes([int(b'\xC0'.hex(), 16) + c]) + t) # Same as above. This if fixes that
                                t = b''
                                c = 2
                                while c < len(buf) and ( buf[c] == buf[0] and (processed_bytes + c < 253) ):
                                        if c == 31: break
                                        c += 1
                                d += bytes([int(b'\xA0'.hex(), 16) + c]) + bytes([buf[0]])
                                buf = buf[c-1:]
                                processed_bytes += c
                                c = 0
                        elif c == 63:
                                d += bytes([int(b'\xC0'.hex(), 16) + c]) + t
                                t = b''
                                processed_bytes += c
                                c = 0
                        else:
                        ...
                        buf = buf[1:]
                if c > 0: d += bytes([int(b'\xC0'.hex(), 16) + c]) + t
        ...
        def readSCB(self, data):
                ...
                for i in data:
                        ...
                        if repeat:
                                ...
                                continue
                        SCB = bytes([i])
                        SCB_type = bytes([int(SCB.hex(), base = 16) & 0xC0])
                        ...
                        if SCB_type == b'\x00':
                                ...
                                sub_type = (int(SCB.hex(), base = 16) & 0xE0)
                                count = (int(SCB.hex(), base = 16) & 0x1F)
                                if sub_type == 0xA0:
                                        repeat = True
                                else:
